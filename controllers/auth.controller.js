...
module.exports.login = async (req, res, next) => {
    try {
        ...
        let token = jwt.sign({ email: existUser.email }, process.env.JWT_SECRET,{ expiresIn: "1h" });
...
module.exports.signup = async (req, res, next) => {
  try {
    let user = req.body;
    let db_user = await pool.query("Select 1 from users where email = ?", [user.email]);
    ...
    let user_created = await pool.query("Insert into users (username,email,password) values (?,?,?)", [
      user.username,
      user.email,
      hashpass,
    ]);
